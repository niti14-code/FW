...
export default function RouteAlerts({ navigate }) {
  ...
  const handleSubscribe = async (e) => {
    ...
    if (!form.pickup || !form.drop)
      return setError('Please select valid pickup and drop locations');
    ...
    try {
      ...
      const res = await api.createAlert({
        ...
        pickup: {
          coordinates: [form.pickup.lng, form.pickup.lat]
        },

        drop: {
          coordinates: [form.drop.lng, form.drop.lat]
        },
  ...
  const handlePostRequest = async (e) => {
    ...
    if (!reqForm.pickup || !reqForm.drop)
      return setError('Please select valid pickup and drop locations');
    ...
    try {
      ...
      await api.createAlert({
        ...
        pickup: {
          coordinates: [reqForm.pickup.lng, reqForm.pickup.lat]
        },

        drop: {
          coordinates: [reqForm.drop.lng, reqForm.drop.lat]
        },

        date: reqForm.date,

        timeRange: {
          start: reqForm.timeStart,
          end: reqForm.timeEnd
        }
  ...
  return (
    ...
    <div className="narrow-wrap fade-up">
      ...
      <h1 className="heading mb-4" style={{fontSize:28}}>
        Route Alerts
      </h1>

      <p className="text-muted mb-24 text-sm">
        Subscribe to alerts for your route, or post a ride request when no rides are found.
      </p>
      ...
      {tab === 'request' && (
        ...
        <div style={{
          ...
          <h3 style={{color:'#fff', marginBottom:4, fontSize:16}}>
            Post a Ride Request
          </h3>
