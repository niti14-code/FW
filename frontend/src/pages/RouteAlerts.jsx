...
export default function RouteAlerts({ navigate }) {
  ...
  const [form, setForm] = useState({ name:'', pickupLat:'', pickupLng:'', dropLat:'', dropLng:'', pickupRadius:5000, dropRadius:5000 });
  const [reqForm, setReqForm] = useState({ pickupLat:'', pickupLng:'', dropLat:'', dropLng:'', date:'', timeStart:'', timeEnd:'' });
  ...
  return (
    <div className="narrow-wrap fade-up">
        ...
        {['subscriptions','request'].map(t => (
          <button key={t} onClick={() => { setTab(t); setError(''); setSuccess(''); }}
            style={{ padding:'8px 18px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:600,
              background: tab===t ? '#6c63ff' : '#222', color: tab===t ? '#fff' : '#aaa' }}>
      ...
      {tab === 'subscriptions' && (
        <div>
          <div style={{background:'#1a1a2e', border:'1px solid #333', borderRadius:12, padding:20, marginBottom:20}}>
            <h3 style={{color:'#fff', marginBottom:16, fontSize:16}}>Create Route Alert</h3>
            <form onSubmit={handleSubscribe}>
              <div className="field"><label>Alert Name</label>
                <input className="input" placeholder="e.g. Home to College" value={form.name} onChange={set('name')} /></div>
              <div className="grid-2">
                <div className="field"><label>Pickup Lat</label>
                  <input className="input" type="number" step="any" value={form.pickupLat} onChange={set('pickupLat')} required /></div>
                <div className="field"><label>Pickup Lng</label>
                  <input className="input" type="number" step="any" value={form.pickupLng} onChange={set('pickupLng')} required /></div>
              </div>
              <div className="grid-2">
                <div className="field"><label>Drop Lat</label>
                  <input className="input" type="number" step="any" value={form.dropLat} onChange={set('dropLat')} required /></div>
                <div className="field"><label>Drop Lng</label>
                  <input className="input" type="number" step="any" value={form.dropLng} onChange={set('dropLng')} required /></div>
              </div>
          ...
          {loading ? <p className="text-muted">Loading...</p> : alerts.length === 0 ? (
            ...
            <div>
              ...
              {alerts.map(a => (
                <div key={a._id} style={{background:'#1a1a2e', border:'1px solid #333', borderRadius:10, padding:16, marginBottom:10, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                  <div>
                    <div style={{color:'#fff', fontWeight:600}}>{a.name || 'Route Alert'}</div>
                    <div style={{color:'#888', fontSize:12}}>Radius: {a.pickupRadius}m pickup / {a.dropRadius}m drop</div>
                  </div>
      ...
      {tab === 'request' && (
        <div style={{background:'#1a1a2e', border:'1px solid #333', borderRadius:12, padding:20}}>
          ...
          <form onSubmit={handlePostRequest}>
            <div className="grid-2">
              <div className="field"><label>Pickup Lat</label>
                <input className="input" type="number" step="any" value={reqForm.pickupLat} onChange={setR('pickupLat')} required /></div>
              <div className="field"><label>Pickup Lng</label>
                <input className="input" type="number" step="any" value={reqForm.pickupLng} onChange={setR('pickupLng')} required /></div>
            </div>
            <div className="grid-2">
              <div className="field"><label>Drop Lat</label>
                <input className="input" type="number" step="any" value={reqForm.dropLat} onChange={setR('dropLat')} required /></div>
              <div className="field"><label>Drop Lng</label>
                <input className="input" type="number" step="any" value={reqForm.dropLng} onChange={setR('dropLng')} required /></div>
            </div>
            <div className="grid-2">
              <div className="field"><label>Date</label>
                <input className="input" type="date" value={reqForm.date} onChange={setR('date')} /></div>
              <div className="field"><label>Preferred Time (from)</label>
                <input className="input" type="time" value={reqForm.timeStart} onChange={setR('timeStart')} /></div>
            </div>
            <button type="submit" className="btn btn-primary btn-full" disabled={actionLoading}>
