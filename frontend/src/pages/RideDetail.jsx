...
// ── ADDED: reverse-geocode fallback for rides that have no stored address ──
...
async function reverseGeocode([lng, lat]) {
  ...
  try {
    ...
    const name = a.neighbourhood || a.suburb || a.village || a.city_district ||
      ...
      `${lat.toFixed(4)}°N ${lat.toFixed(4)}°E`;
...
// Reads stored address first; falls back to reverse geocoding for old rides
...
export default function RideDetail({ navigate, rideId }) {
  ...
  // ADDED: resolve location names reactively
  ...
  useEffect(() => {
    ...
    api.getRide(rideId)
      // FIXED: API returns { ride, recurringInstances } — extract .ride
  ...
  // CHANGED: use resolved names instead of raw coordinates
  const pickup = pickupName || 'Loading…';
  const drop   = dropName   || 'Loading…';
...
// Reverse-geocode coords as a fallback for old rides with no address stored
const geoCache = new Map();
async function reverseGeocode([lng, lat]) {
  const key = `${lat.toFixed(4)},${lng.toFixed(4)}`;
  if (geoCache.has(key)) return geoCache.get(key);
...
export default function RideDetail({ navigate, rideId }) {
  ...
  useEffect(() => {
    ...
    api.getRide(rideId)
      .then(data => {
        // API returns { ride, recurringInstances } — must extract .ride
  ...
  const isOwner  = provider?._id === user?.id || provider?._id === user?._id || ride.providerId === user?.id;
  const isSeeker = user?.role === 'seeker' || user?.role === 'both';

  const dateStr = ride.date
    ? new Date(ride.date).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
    : 'Date not set';
      ...
      <div className="card">
       { Header }
        ...
        <div className="card-body">
          ...
          <div className="rd-route mb-24">
            <div className="rd-stop">
              ...
              <div>
                ...
                <div className="rd-loc">
                  {pickupName || <span style={{ color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>Loading…</span>}
                </div>
              </div>
            </div>
            <div className="rd-connector" />
            <div className="rd-stop">
              ...
              <div>
                <div className="text-dim text-xs mb-6">DROP</div>
                <div className="rd-loc">
                  {dropName || <span style={{ color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>Loading…</span>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid-2 mb-24">
            {[
              { icon: '📅', label: 'Date',       val: dateStr,                       accent: false },
              { icon: '🕐', label: 'Time',       val: ride.time || '—',              accent: false },
              { icon: '💺', label: 'Seats Left', val: ride.seatsAvailable != null ? `${ride.seatsAvailable} available` : '—', accent: false },
              { icon: '₹',  label: 'Cost/Seat',  val: ride.costPerSeat != null ? `₹${ride.costPerSeat}` : '—', accent: true },
            ].map(item => (
              <div key={item.label} className="info-box">
                <div className="text-dim text-xs mb-6">{item.icon} {item.label}</div>
                <div className={`info-val ${item.accent ? 'text-accent' : ''}`}>{item.val}</div>
          ...
          {provider && typeof provider === 'object' && (
            <div className="provider-box mb-24">
              <div className="prov-ava">{provider.name?.charAt(0)?.toUpperCase() || 'P'}</div>
              <div>
                <div className="prov-name">{provider.name || 'Provider'}</div>
                {provider.phone && <div className="text-muted text-sm mt-4">📞 {provider.phone}</div>}
                {provider.rating > 0 && <div className="text-muted text-sm mt-4">⭐ {provider.rating}</div>}
              </div>
            </div>
          )}
          ...
          { Recurring badge }
          {ride.isRecurring && (
            <div className="alert mb-16" style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', color: '#a5b4fc' }}>
          ...
          <div className="flex gap-12 flex-wrap">
            {isSeeker && !isOwner && !booking.status && (ride.seatsAvailable ?? 0) > 0 && (
              <button
                className={`btn btn-primary btn-lg ${booking.loading ? 'btn-loading' : ''}`}
                onClick={handleBook}
            ...
            {(ride.seatsAvailable ?? 0) === 0 && !booking.status && (
              <span className="badge badge-rejected" style={{ fontSize: 13, padding: '8px 14px' }}>
                No seats available
              </span>
            )}
            {isOwner && (
