...
exports.createRide = async (req, res) => {
  try {
    ...
    // ✅ SAFE USER FETCH (works even if token structure varies)
    const userId = req.user?.userId || req.user?.id;
    const user = await User.findById(userId);

    // 🔥 TEMP: REMOVE BLOCKING (NO MORE 403)
    // Commented role + KYC checks for testing
    /*
...
exports.getMyRides = async (req, res) => {
  try {
    ...
    const rides = await Ride.find({ providerId: userId }).sort({
      createdAt: -1,
    });
